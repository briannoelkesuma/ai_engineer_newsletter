...
def update_video_status(video_id: str, status: str):
    supabase = get_db_client()
    response = supabase.table("videos").update({"status": status}).eq("video_id", video_id).execute()
    return response.data

def insert_insights(video_id: str, company: str, high_level_summary: str, detailed_learnings: str, newsletter_text: str):
    supabase = get_db_client()
    data = {
        "video_id": video_id,
        "company": company,
